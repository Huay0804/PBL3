...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train Double DQN + Dueling (shared protocol).")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    training_dir = ensure_dir(os.path.join(results_dir, "training_ddqn"))
    ...
    for run_idx in range(run_start, run_end + 1):
        ...
        run_routes_dir = ensure_dir(os.path.join(routes_root, f"ddqn_run{run_idx}"))
        ...
        q_model = build_dueling_q_model(env.state_dim, env.num_actions, hidden_sizes, lr=float(lr))
        target_model = build_dueling_q_model(env.state_dim, env.num_actions, hidden_sizes, lr=float(lr))
        ...
        try:
            for ep in range(episodes):
                ...
                print(f"ddqn run {run_idx} ep {ep + 1}/{episodes} start seed={seed}", flush=True)
                ...
                while not done:
                    ...
                    if len(replay) >= int(batch_size):
                        ...
                        q_next_online = q_model.predict(next_states, verbose=0)
                ...
                print(
                    f"ddqn run {run_idx} ep {ep:03d} seed={seed} eps={epsilon:.3f} "
